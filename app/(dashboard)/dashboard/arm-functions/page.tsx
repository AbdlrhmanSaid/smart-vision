...
export default function ArmFunctionsPage() {
  ...
  const filteredFunctions = armFunctions?.filter(
    (func) => func.name.toLowerCase().includes(searchQuery.toLowerCase())
  ) || [];

  const getTypeInfo = (type: string) => {
    switch (type) {
      case "frames":
        return { icon: Video, color: "text-blue-500", bg: "bg-blue-500/10", label: "إطارات حركية" };
      case "steps":
        return { icon: Layers, color: "text-amber-500", bg: "bg-amber-500/10", label: "خطوات مبرمجة" };
      case "sequence":
        return { icon: ListOrdered, color: "text-purple-500", bg: "bg-purple-500/10", label: "تسلسل أوامر" };
      default:
        return { icon: Box, color: "text-slate-500", bg: "bg-slate-500/10", label: "غير معروف" };
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-3xl border border-border shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">أوامر الذراع الآلي</h1>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <p className="text-muted-foreground mt-6 font-medium animate-pulse">جاري تحميل الأوامر...</p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-card rounded-3xl border border-dashed border-border shadow-sm">
          ...
          <p className="text-lg font-medium text-foreground">لا يوجد أوامر متاحة</p>
          <p className="text-sm text-muted-foreground mt-1">جرب البحث بكلمة مختلفة</p>
        </div>
      ) : (
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  isCurrentlyRunning ? "border-primary shadow-primary/20 shadow-lg" : "border-border hover:border-primary/40 hover:shadow-md"
                )}
              >
                ...
                <div className="flex justify-between items-start mb-5 relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-foreground capitalize line-clamp-1" title={func.name}>{func.name}</h3>
                ...
                <div className="bg-muted/40 border border-border/50 rounded-2xl p-4 mb-5 relative z-10 flex-1">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground font-medium">النقاط/الخطوات:</span>
                    <span className="font-bold bg-background px-2 py-0.5 rounded-md border border-border/50 shadow-xs text-foreground">
                      {func.type === 'frames' ? func.frames?.length || 0 : func.steps?.length || 0}
                    </span>
                  </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isPending && !isCurrentlyRunning && "opacity-50 cursor-not-allowed"
                  )}
                >
                  {isCurrentlyRunning ? (
                    <>
                      <div className="absolute inset-0 bg-white/20 animate-pulse" />
                      <Activity className="size-4 animate-spin" /> جاري التنفيذ...
                    </>
                  ) : (
                    <>
                      <Play className="size-4 group-hover:scale-110 transition-transform" /> تشغيل الأمر
                    </>
                  )}
                </button>
              </div>
